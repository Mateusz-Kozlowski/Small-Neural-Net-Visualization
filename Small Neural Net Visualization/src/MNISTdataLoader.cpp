...
unsigned char Utils::MNISTdataLoader::readChar(std::ifstream& file)
{
	char c;
	file.read(&c, 1);
	return c;
...
int Utils::MNISTdataLoader::readInt(std::ifstream& file)
{
	int result = 0;
...
std::vector<std::vector<Scalar>> Utils::MNISTdataLoader::readMnistImages(const char* filename)
{
...
std::vector<int> Utils::MNISTdataLoader::readMnistLabels(const char* filename)
{
	std::ifstream file(filename, std::ios::binary);
...
void Utils::MNISTdataLoader::showImage(const std::vector<Scalar>& image)
{
...
std::vector<std::vector<Scalar>> Utils::MNISTdataLoader::reorganizeLabels(const std::vector<int>& labels)
{
